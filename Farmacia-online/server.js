// Farmacia-online/server.js
const express = require('express');
const cors = require('cors');
const { createClient } = require('@supabase/supabase-js');

const app = express();
const PORT = process.env.PORT || 3000; // Define a porta do servidor

// SUAS CHAVES DO SUPABASE AQUI - COPIE E COLE DA SEÇÃO 'API Keys' DO SEU PROJETO SUPABASE
const SUPABASE_URL = 'SUA_PROJECT_URL_AQUI'; https://vovyubntbeesdwibrdlf.supabase.co
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InZvdnl1Ym50YmVlc2R3aWJyZGxmIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTI1OTk5ODcsImV4cCI6MjA2ODE3NTk4N30.j0l9wfwuTaX9bOFq8G3dXd1_y3MqYUjQfzZlouavn9s'; // Ex: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFiY2RlZmdo...';

// Cria o cliente Supabase
const supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY);

// Middleware para permitir requisições de diferentes origens (CORS)
app.use(cors());
// Middleware para parsear JSON no corpo das requisições
app.use(express.json());

// Rota inicial para testar se o servidor está rodando
app.get('/', (req, res) => {
    res.send('Bem-vindo à API da Farmácia Online (Supabase)!');
});

// Rota para buscar TODOS os produtos
app.get('/api/products', async (req, res) => {
    try {
        // Tenta buscar os dados da tabela 'produtos' no Supabase
        const { data: products, error } = await supabase
            .from('produtos') // Nome da sua tabela no Supabase
            .select('*'); // Seleciona todas as colunas

        // Se houver um erro na comunicação com o Supabase
        if (error) {
            console.error('Erro ao buscar produtos do Supabase:', error); // Loga o erro COMPLETO no terminal do backend
            // Retorna um status 500 para o frontend
            return res.status(500).send('Erro interno do servidor ao buscar produtos.');
        }

        // Se tudo deu certo, retorna os produtos como JSON para o frontend
        res.json(products);
    } catch (error) {
        // Captura erros inesperados no processo da rota
        console.error('Erro inesperado na rota /api/products:', error);
        res.status(500).send('Erro interno do servidor.');
    }
});

// Rota para buscar um produto específico por ID (opcional por enquanto, mas útil)
app.get('/api/products/:id', async (req, res) => {
    try {
        const productId = req.params.id;
        const { data: product, error } = await supabase
            .from('produtos')
            .select('*')
            .eq('id', productId) // Filtra pelo ID
            .single(); // Espera um único resultado

        if (error && error.code !== 'PGRST116') { // PGRST116 é para "No rows found"
            console.error('Erro ao buscar produto por ID no Supabase:', error);
            return res.status(500).send('Erro interno do servidor ao buscar produto por ID.');
        }

        if (product) {
            res.json(product);
        } else {
            res.status(404).send('Produto não encontrado');
        }
    } catch (error) {
        console.error('Erro inesperado na rota /api/products/:id:', error);
        res.status(500).send('Erro interno do servidor.');
    }
});

// Rota para adicionar um novo produto (exemplo, para testes)
app.post('/api/products', async (req, res) => {
    try {
        const { data, error } = await supabase
            .from('produtos')
            .insert([req.body]) // Insere os dados do corpo da requisição
            .select(); // Retorna o item inserido

        if (error) {
            console.error('Erro ao adicionar produto ao Supabase:', error);
            return res.status(400).send('Erro ao adicionar produto. Verifique os dados.');
        }
        res.status(201).json(data[0]);
    } catch (error) {
        console.error('Erro inesperado na rota POST /api/products:', error);
        res.status(500).send('Erro interno do servidor.');
    }
});

// Rota para lidar com pedidos (exemplo de como seria)
app.post('/api/orders', (req, res) => {
    const order = req.body;
    console.log('Novo pedido recebido (via Supabase):', order);
    res.status(201).json({ message: 'Pedido recebido com sucesso!', orderId: Date.now() });
});

// Inicia o servidor e o faz escutar na porta definida
app.listen(PORT, () => {
    console.log(`Servidor rodando em http://localhost:${PORT}`);
    console.log(`API de produtos disponível em http://localhost:${PORT}/api/products`);
});